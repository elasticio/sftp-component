...
const { fillOutputBody } = require('../utils/utils');
...
class LookupFiles extends LookupObjects {
  ...
  async getObjectsByCriteria(objectType, criteria, msg, cfg) {
    ...
    if (cfg.uploadFilesToAttachments === 'No') {
      if (cfg.emitFileContent) {
        const filledFilesList = [];
        for (const file of filteredList) {
          const base64Content = await getFileContentInBase64(this, file, msg.body[DIR]);
          filledFilesList.push(base64Content.body);
        }
        return filledFilesList;
      }
      ...
      do {
        try {
          ...
          fileUploadResult = await Promise.race([
            this.uploadAttachmentAndProcessFileContent(this, cfg, file, msg.body[DIR], token),
  ...
  async uploadAttachmentAndProcessFileContent(context, cfg, file, directory, token) {
    const { emitFileContent } = cfg;
    if (emitFileContent) {
      context.logger.info('Getting file content as a Base64 string...');
      await getFileContentInBase64(this, file, directory);
    }
    context.logger.info('Uploading file content to the attachment...');
    const { body, attachments } = await uploadFromSftpToAttachmentBuffer(this, file, directory, token);
...
async function process(msg, cfg, snapshot = {}) {
  this.logger.info('===PROCESS FUNCTION START===');
  try {
    ...
    if (!sftpClient.isConnected()) {
      this.logger.info('===SFTP CLIENT NOT CONNECTED. CONNECTING...===');
      await sftpClient.connect();
    }
    ...
    await lookupFilesAction.process(msg, cfg, snapshot);
    //=====
    const memoryUsage = global.process.memoryUsage();

    this.logger.info('Memory Usage:');
    this.logger.info(`RSS: ${Math.round(memoryUsage.rss / 1024 / 1024)} MB`);          // Resident Set Size
    this.logger.info(`Heap Total: ${Math.round(memoryUsage.heapTotal / 1024 / 1024)} MB`); // Total heap allocated
    this.logger.info(`Heap Used: ${Math.round(memoryUsage.heapUsed / 1024 / 1024)} MB`);   // Heap used
    this.logger.info(`External: ${Math.round(memoryUsage.external / 1024 / 1024)} MB`);    //
  } catch (e) {
...
async function init(cfg) {
  ...
  logger.info('===INIT HOOK START===');
  sftpClient = new Sftp(getLogger(), cfg);
  await sftpClient.connect();
  logger.info('===INIT HOOK END===');
}
