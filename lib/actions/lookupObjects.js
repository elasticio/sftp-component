/* eslint-disable no-underscore-dangle, camelcase, no-param-reassign, no-unused-vars, no-restricted-syntax */
...
const FILE_UPLOAD_RETRY = 2;
const RETRY_TIME_OUT = 10000;
const FILE_UPLOAD_TIME_OUT = 60000;
      ...
      do {
        try {
          fileUploadResult = await Promise.race([
            uploadFromSftpToAttachmentBuffer(this, file, msg.body[DIR]),
            timeOut(FILE_UPLOAD_TIME_OUT),
          ]);
          break;
        } catch (e) {
          currentTry++;
          if (currentTry > FILE_UPLOAD_RETRY) throw e;
          this.logger.error(`got error ${e.message}, going to retry (${currentTry} of ${FILE_UPLOAD_RETRY}) upload file`);
          await sleep(RETRY_TIME_OUT);
