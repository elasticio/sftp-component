...
exports.process = async function processAction(msg, cfg) {
  ...
  const filename = msg.body.filename
    ...
    ? (msg.attachments.length > 1 ? msg.body.filename.replace('\.[1-9a-zA-Z]+$') : msg.body.filename)
    : null;
  this.logger.info(`Prepared filename: ${msg.attachments.length}`);
  ...
  this.logger.info(`Found ${msg.attachments.length} attachments`);
  ...
  for (const key of Object.keys(msg.attachments)) {
    ...
    const keyName = filename
      ? (msg.attachments.length > 1 ? filename + key : filename)
      : key;
