...
async function addAttachment(msg, name, getStream, contentLength) {
  try {
    ...
    const attachmentId = await new AttachmentProcessor().uploadAttachment(getStream);
    const attachmentUrl = new AttachmentProcessor().getMaesterAttachmentUrlById(attachmentId);
