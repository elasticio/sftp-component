...
const MAX_FILE_SIZE = process.env.MAX_FILE_SIZE || 5000000;

async function addAttachment(msg, name, stream, contentLength) {
  try {
    if (contentLength > MAX_FILE_SIZE) {
      throw new Error(`File is ${contentLength} bytes, and is too large to upload as an attachment`);
    }
...
async function uploadFromSftpToAttachment(context, body, dir) {
  ...
  if (fileSize > MAX_FILE_SIZE) {
    throw new Error(`File is ${fileSize} bytes, and is too large to upload as an attachment`);
  }
