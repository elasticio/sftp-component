...
async function uploadFromSftpToAttachmentBuffer(context, body, dir) {
  ...
  const filePath = path.join(dir, body.name);
  const fileSize = body.size;
  const REQUEST_MAX_CONTENT_LENGTH = process.env.REQUEST_MAX_CONTENT_LENGTH ? parseInt(process.env.REQUEST_MAX_CONTENT_LENGTH, 10) : 104857600; // 10MB
  console.log(fileSize, MAX_FILE_SIZE, REQUEST_MAX_CONTENT_LENGTH);
  console.log(fileSize > MAX_FILE_SIZE, fileSize > REQUEST_MAX_CONTENT_LENGTH);
  if (fileSize > MAX_FILE_SIZE) {
    throw new Error(`File size is ${fileSize} bytes, it violates the variable MAX_FILE_SIZE, which is currently set to ${MAX_FILE_SIZE} bytes`);
  }
  ...
  const attachmentUrl = `${uploadResult.config.url}${uploadResult.data.objectId}`;
  ...
  body.modifyTime = unixTimeToIsoDate(body.modifyTime);
  body.accessTime = unixTimeToIsoDate(body.accessTime);
  body.objectId = uploadResult.data.objectId;
  return { body, attachments };
}
