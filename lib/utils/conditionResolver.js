...
class ConditionResolver {
  ...
  processCondition(searchTerm, item, properties) {
    ...
    function checkAndCastIfDate() {
      if (properties.dateFields.includes(fieldName)) {
        return Number(moment(fieldValue, moment.ISO_8601, true).format('x'));
      }
      return fieldValue;
    }
  ...
  processConditions(msg, item, properties = { dateFields: [] }) {
    ...
    ors.forEach((or, j) => {
      ...
      for (let i = from; i > to; i--) {
        ...
        andResult.push(this.processCondition(msg[`searchTerm${i}`], item, properties));
      }
