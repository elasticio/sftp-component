  ...
  async emitIndividually(files) {
    ...
    for (let i = 0; i < files.length; i += 1) {
      ...
      try {
        ...
        const attachmentResult = await uploadFromSftpToAttachment(context, file, this.cfg.directory);
        const resultMessage = messages.newMessageWithBody(this.prepareMessageDescription(file));
