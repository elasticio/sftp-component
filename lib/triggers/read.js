...
const awsAttachments = require('../awsAttachments.js');
    ...
    client.opendir(dir, function readdir(err, handle) {
      ...
      readDir(client, dir, function (err, list) {
        ...
        try {
          ...
          var functions = createReaders(cfg, client, dir, files, processedFolderExist, self);
  ...
  var candidates = _.filter(files, function (current) {
    ...
    var attrs = current.attrs;
        ...
        if (isDirectory(current)) {
            ...
            console.log('WARNING FILE: %s will not be uploaded since it size=%s exceeds maximum size of %s', current.filename,
...
function createReaders(cfg, client, dir, files, processedFolderExist, emitter) {
  ...
  return _.map(files, function (file) {
    return function (cb) {
      createMessageForFile(cfg, client, dir, file, processedFolderExist, function (err, msg) {
        if (err) {
          return cb(err);
        }
...
function createMessageForFile(cfg, client, dir, file, processedFolderExist, cb) {
  ...
  move(client, dir, file, processedFolderExist, function (err, newPath) {
    ...
    if (process.env.ATTACHMENT_TO_AWS === 'true' || cfg.attachmentsToAws || cfg.attachmentsToAws === 'true') {
      awsAttachments.addAttachment(cfg, msg, fileName, readStream, file.attrs.size)
        .then(function () {
          cb(null, msg);
