...
exports.process = async function process(msg, cfg) {
  ...
  this.logger.debug('Found the %s files: ', Object.keys(list.filter((item) => item.type === '-')).length);
  const files = await filterFilesByPattern(list, cfg.pattern);
  this.logger.debug(`Files that match filter ${cfg.pattern}: `, Object.keys(files.map((file) => file.name)).length);
