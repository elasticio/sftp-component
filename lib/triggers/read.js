...
exports.process = async function process(msg, cfg) {
  ...
  this.logger.debug('Found the following files: ', list.filter((item) => item.type === '-').map((item) => item.name));
  const files = await filterFilesByPattern(list, cfg.pattern);
  this.logger.debug(`Files that match filter ${cfg.pattern}: `, files.map((file) => file.name));
