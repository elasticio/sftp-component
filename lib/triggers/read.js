...
function logProcessedFiled(results) {
    ...
    const fileNames = _.map(results, function (next) {
...
function doesProcessedFolderExist(files) {
    ...
    const result = _.find(files, function (current) {
...
function resolveFiles(files, cfg) {
    ...
    const candidates = _.filter(files, function (current) {
